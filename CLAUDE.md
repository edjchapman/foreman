...
- `make deploy VERSION=<x.y.z>` — deploy to Railway (`scripts/railway-deploy.sh`: pins the semver image tag per service via the GraphQL API; web deploys first and gates worker/beat behind pre-deploy `migrate` + `/readyz`). Runs automatically from the `deploy` job in `release-please.yml` on each release; needs `RAILWAY_TOKEN` (secret) + `RAILWAY_{ENVIRONMENT,WEB_SERVICE,WORKER_SERVICE,BEAT_SERVICE}_ID` (repo variables). Topology/provisioning/rollback: `docs/deploy.md`; rationale: ADR 0005.
